...
const Tuple sep;
  ...
  public: 
    ...
    BufferElement<Tuple> get()
    {
      ...
      // else if(buffiter->atEnd())
      // {
      //   std::cerr<<"Buffer iterator at End";
      //   assert(0);
      //   exit(-1);
      // }
      //if(iter.get != NULL)
      //std::cout<<"iter  init in FI"<<std::endl;
      if(iter->isEmpty() && !nodeStack.empty())
      {
        //std::cout<<"node iter is pointing to is empty"<<std::endl;
        node = nodeStack.top();
        iter = node->iterator();
      }
      return iter->get();
    ...
    void next()
    {
      ...
      if(node != NULL || !nodeStack.empty())
      {
        ...
        {
          //std::cout<<"found Btree cog in next"<<std::endl;
          nodeStack.push(node->rhs_ptr());
          node = node->lhs_ptr();
    ...
    void seek(const Tuple &k)
    {
      ...
      while(node!=NULL)
      {
        ...
        else if(node->type() == COG_SORTED_ARRAY)
        {
          ...
          // {
            //std::cout<<"before buffer seek"<<std::endl;
            iter->seek(k);
            return;
          // }
          // else
        ...
        else if(node->type() == COG_ARRAY)
        {
          ...
          // if(!iter->isEmpty() || iter.get() == NULL)
          // {
            //std::cout<<"buffer is not empty before seek"<<std::endl;
            iter->seek(k);
            break;
          //}
        }
